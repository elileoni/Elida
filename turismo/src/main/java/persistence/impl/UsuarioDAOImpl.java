...
public class UsuarioDAOImpl implements UsuarioDAO {
	...
	public List<Producto> filtrarProductos(Usuario user) throws SQLException {
		AttractionService list = new AttractionService();
		List<Atracciones> atracList = new ArrayList<Atracciones>();
		atracList = list.list();
		...
		while (resultado.next()) {
			if (resultado.getInt(3) == 0) {
				...
				prodFiltrada.add(buscarAtraccion(idAtraccion, atracList));
			}
		}
